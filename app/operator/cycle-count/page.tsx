...
export default async function CycleCountPage() {
  ...
  return <div className="page">
    ...
    <section className="grid grid-3">
      ...
      <div><div className="section-header"><h2 className="section-title">Input aktual</h2></div><WmsActionForm action={submitCycleCountAction} submitLabel="Simpan Hitungan"><label className="wms-field"><span>Baris Count *</span><select name="cycle_count_line_id" required><option value="">Pilih baris</option>{rows.filter(l => !l.is_counted).map(l => <option key={l.id} value={l.id}>{l.location?.location_code} · {l.lpn?.lpn_code} · buku {formatKg(l.expected_qty_kg)}</option>)}</select></label><label className="wms-field"><span>Kuantitas Aktual (KG) *</span><input min="0" name="actual_qty_kg" placeholder="0.000" required step="0.001" type="number" /></label></WmsActionForm></div>
    ...
    <section className="section"><div className="section-header"><div><h2 className="section-title">Ajukan koreksi di luar sesi</h2><p className="section-subtitle">Memerlukan persetujuan Supervisor dan tidak langsung mengubah saldo.</p></div></div><WmsActionForm action={requestAdjustmentAction} submitLabel="Ajukan Adjustment"><label className="wms-field"><span>Token LPN *</span><input name="lpn_token" placeholder="Scan atau masukkan token LPN" required /></label><label className="wms-field"><span>Saldo Target (KG) *</span><input min="0" name="target_qty_kg" placeholder="0.000" required step="0.001" type="number" /></label><label className="wms-field"><span>Alasan *</span><select name="reason_code"><option>COUNT_VARIANCE</option><option>DAMAGE</option><option>QUALITY_HOLD</option><option>DATA_CORRECTION</option></select></label><label className="wms-field wide"><span>Catatan</span><input name="note" placeholder="Jelaskan alasan koreksi" /></label></WmsActionForm></section>
